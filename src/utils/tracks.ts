...
export function useSoundcloudTracksSearch(query: string | null, tracksLimit: number, saveSearch: boolean = true) {
	const [soundcloudTracks, setData] = useState<DbTrack[]>([]);
	...
	useEffect(() => {
		const fetchResults = async () => {
			...
			try {
				const { status, data } = await apiRequest(
					`${process.env.NEXT_PUBLIC_API_URL!}/api/v1/tracks/search/${encodeURIComponent(query)}?limit=${tracksLimit}&save=${saveSearch}`,
					undefined,
					ResponseDataType.JSON,
					true
				);
				if (status != 200 || data == null || data.results == null || data.status == 'error') {
					if (data.error) throw data.error;
					throw 'An error has occured while playing track';
				}
				setData(data.results as DbTrack[]);
			} catch (err) {
				setData([]);
			} finally {
				setLoading(false);
	...
	useEffect(() => {
		(async () => {
			try {
				...
				if (isValidPlayerPreferences(playerPreferences)) setPlayerPreferences((prev) => ({ ...prev, ...playerPreferences }));
