...
export async function playTrackByQueueId(playerId: string, queueId: string) {
  const url = new URL(
    `${process.env.NEXT_PUBLIC_API_URL!}/api/v1/player/${playerId}/tracks/queue/${encodeURIComponent(queueId)}`,
  );
...
export async function playSoundcloudTrack(
  ...
  const url = new URL(
    `${process.env.NEXT_PUBLIC_API_URL!}/api/v1/player/${playerId}/tracks/soundcloud/${encodeURIComponent(trackUrl)}`,
  );
  ...
  if (status != 200 || data == null || data.status == "error") {
...
export async function likeTrack(trackId: string, trackProvider: string) {
  const url = new URL(
    `${process.env.NEXT_PUBLIC_API_URL!}/api/v1/users/me/likes/${encodeURIComponent(trackId)}`,
  );
  const { data } = await apiRequest(
    url.href,
    {
...
export function useSoundcloudTracksSearch(
  query: string | null,
  tracksLimit: number,
) {
  const [soundcloudTracks, setData] = useState<Track[]>([]);
  const [loading, setLoading] = useState(true);
  useEffect(() => {
    const fetchResults = async () => {
      if (!query || query.length == 0) return setData([]);
      setLoading(true);
      try {
        const { status, data } = await apiRequest(
          `${process.env.NEXT_PUBLIC_API_URL!}/api/v1/tracks/search/${encodeURIComponent(query)}?limit=${tracksLimit}`,
          undefined,
          ResponseDataType.JSON,
          true,
        );
  ...
  return { soundcloudTracks, loading };
}
...
export function useCurrentTrack(playerId: string) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const { data } = await apiRequest(
          `${process.env.NEXT_PUBLIC_API_URL}/api/v1/player/${playerId}/tracks/queue/current`,
          {
            method: "GET",
            cache: "no-cache",
          },
        ...
        const { status, queueTrack, playerStatus, playerPreferences } =
          data as QueueTrackResponse;
...
export function usePlayerQueue(playerId: string) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const { status, data } = await apiRequest(
          `${process.env.NEXT_PUBLIC_API_URL}/api/v1/player/${playerId}/tracks/queue/`,
          {
            method: "GET",
            cache: "no-cache",
          },
...
export function useLikedTracks() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const { status, data } = await apiRequest(
          `${process.env.NEXT_PUBLIC_API_URL}/api/v1/users/me/likes`,
          {
            method: "GET",
            cache: "no-cache",
          },
