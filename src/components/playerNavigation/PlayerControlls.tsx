...
import {
  ...
  repeat,
  resume,
  volume as updatePlayerVolume,
  volume,
} from "@/utils/controlls";
...
export default function PlayerControlls({
  className,
  guildId,
}: {
  className: string;
  ...
  useEffect(() => {
    ...
    handler.subscribe("repeat", (repeat: Repeat) =>
      setPlayerPreferences((prev) => ({ ...prev, repeat })),
    );
  ...
  const repeatPress = async (rep: Repeat) => {
    try {
      if (await repeat(guildId, rep))
        setPlayerPreferences((prev) => ({ ...prev, repeat: rep }));
    } catch (err) {
      console.error(err);
      pushAlert(String(err));
    }
  ...
  const updateVolume = async (volume: number) => {
    try {
      if (await updatePlayerVolume(guildId, volume))
        setPlayerPreferences((prev) => ({ ...prev, volume }));
    } catch (err) {
      console.error(err);
      pushAlert(String(err));
    }
  ...
  return (
    <div
      className={`${className} flex bg-black-default z-10 shadow-md items-center px-6`}
      style={{ height: "80px" }}
    >
      <div className="flex items-center gap-7 w-full">
        ...
        <div className="flex items-center gap-2">
          ...
          <PlayerRepeatButton
            onClick={repeatPress}
            repeat={playerPreferences?.repeat}
          />
        </div>
        <div className="ml-auto flex items-center">
          ...
          <PlayerVolumeSlider
            ...
            defaultVolume={playerPreferences.volume}
          />
        </div>
      </div>
    </div>
...
function PlayerRepeatButton({
  ...
  return (
    <div className="flex items-center justify-center relative">
      ...
      />
      {repeat === "queue" && (
        <div className="absolute w-1 h-1 rounded-full bg-blue-light transition-all"></div>
      )}
    </div>
  );
}
  ...
  return (
    ...
    >
      <Icon
        className={
          active
            ? "text-blue-light transition-all"
            : "text-white-gray transition-all"
        }
        style={style}
      />
    </button>
