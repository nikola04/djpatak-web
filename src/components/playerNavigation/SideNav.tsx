	...
	return (
		<div ref={profileMenuRef} className="m-2 relative">
			<div
				...
				className={`relative flex md:w-52 w-48 overflow-hidden border border-transparent items-center bg-blue-dark rounded-md px-2 py-1.5 cursor-pointer hover:border-blue-light transition-all duration-200 active:ring-blue ring-[2px] ${open ? 'ring-blue' : 'ring-transparent'}`}
			>
				<div style={{ padding: '2px' }}>
					<GuildIcon guild={selectedGuild} size={24} />
				</div>
				<p className="text-white-gray px-2 text-base text-nowrap text-ellipsis overflow-hidden" title={selectedGuild.name}>
					{selectedGuild.name}
				</p>
			</div>
			{open && (
				<div className="absolute left-0 p-1.5 mt-2 w-52 rounded-lg z-20 overflow-hidden bg-blue-grayish shadow-md">
					{userGuilds.map((guild: DiscordGuild, ind) => (
						<div
							onClick={() => router.push('/player/' + guild.id)}
							key={ind}
							className="flex w-full items-center top-0 p-1 hover:bg-white-hover active:bg-white-active rounded-md cursor-pointer transition-all"
						>
							<div style={{ padding: '2px' }}>
...
function GuildSelectorSceleton() {
	return (
		<div className="m-2 flex flex-col">
			<div className="flex items-center bg-blue-dark rounded-md px-2 py-1.5 w-48 md:w-52 border border-transparent">
				<div style={{ padding: '2px' }}>
...
function NavItem({ data, guildId, pathname }: { data: LinkType; guildId: string; pathname: string }) {
	...
	return (
		<Link
			title={data.name}
			href={url}
			className={`my-0.5 py-1.5 rounded-md min-w-40 px-2 ${!isActive(url) && 'hover:bg-blue-grayish'} active:bg-opacity-90 transition-all bg-opacity-100 ${isActive(url) && 'bg-blue-light'}`}
		>
			<span>
