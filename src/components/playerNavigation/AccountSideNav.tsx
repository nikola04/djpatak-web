...
function NavItem({ data, pathname }: { data: LinkType; pathname: string }) {
	...
	return (
		<Link
			title={data.name}
			href={data.href}
			className={`my-0.5 py-[7px] rounded-r-2xl w-56 px-2 dark:text-white-gray text-black-light ${!isActive(data.href) && 'hover:bg-white-hover dark:hover:bg-blue-grayish active:bg-white-active dark:active:bg-black-hover'} active:bg-opacity-90 transition-all bg-opacity-100 ${isActive(data.href) && 'bg-blue-active !text-black-light'}`}
		>
			<span>
