...
export function GuildIcon({ guild, size, style, styleNoImage, styleWithImage, className, classNameNoImage, classNameNoImageText, classNameWithImage }:{
    guild: DiscordGuild,
    size: number,
    style?: CSSProperties,
    ...
    classNameNoImageText?: React.ComponentProps<'p'>['className']
    ...
    return <div style={{ width: `${size}px`, height: `${size}px`, ...style, ...styleNoImage }} className={`flex items-center justify-center border-2 border-black-default rounded-full ${className} ${classNameNoImage}`}>
        <p className={`text-center font-bold text-sm text-white-default ${classNameNoImageText}`}>{ guild.name[0].toUpperCase() }</p>
    </div>
}