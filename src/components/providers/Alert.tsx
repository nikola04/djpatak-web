...
export function AlertProvider({ children }: { children: ReactNode }) {
	...
	const pushAlert = useCallback(
		(alert: string, error: boolean = true) =>
			setAlerts((prev) => {
				return [
					...prev,
					{
			...
			<div className="fixed z-50 bottom-16 right-0">
				<div className="relative m-6 flex flex-col transition-all duration-[320] ease">
					{alerts.map(({ id, message, isError, timeRemaining }) => (
						<Alert key={id} message={message} isError={isError} onCloseAlert={() => dropAlert(id)} shouldDrop={timeRemaining < 1} />
					))}
				</div>
			</div>
...
function Alert({ message, isError, shouldDrop, onCloseAlert }: { message: string; isError: boolean; onCloseAlert: () => any; shouldDrop: boolean }) {
			...
			>
				<div className="flex items-center px-1 py-1.5 w-full">
					...
					<div className="ml-auto pl-1">
						<SmallIconButton className="mx-1" title="Close" icon={<IoClose className="text-2xl" />} onClick={onCloseAlert} />
					</div>
				</div>
				<div className={`w-full h-[3px] ${isError ? 'bg-red-700' : 'bg-green-700'}`}></div>
			</div>
		)
	);
}
