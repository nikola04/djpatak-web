...
import { PrimaryButton, SmallIconButton } from "../Buttons";
...
export const PlaylistPopupContent = ({ initialValues = { name: "", desc: "" }, onClose, submit, submitType }: {
    initialValues?: {
        name: string,
        desc: string
    },
    ...
    return <DefaultPopupContainer>
        ...
        <div className="flex flex-col py-2">
            <form className="flex flex-col" onSubmit={onSubmit}>
                <input value={newPlaylistName} onInput={e => setNewPlaylistName((e.target as HTMLInputElement).value)} className="bg-[#2b2b36] border-1 border-transparent focus:border-blue-light text-white-default text-base rounded !outline-none py-2 px-3 font-thin mb-2 mt-0.5" placeholder="Playlist name" type="text" id="playlistName" name="playlist-name"/>
                <label htmlFor="playlistDesc" className="text-white-gray text-sm opacity-70">Optional</label>
                <input value={newPlaylistDescription} onInput={e => setNewPlaylistDescription((e.target as HTMLInputElement).value)} className="bg-[#2b2b36] border-1 border-transparent focus:border-blue-light text-white-default text-base rounded !outline-none py-2 px-3 font-thin mb-2 mt-0.5" placeholder="Playlist description" type="text" id="playlistDesc" name="playlist-description"/>
                <PrimaryButton value={submitType === 'create' ? "Create" : "Save"} type="submit" className="mr-auto mt-3.5" disabled={isLoading} icon={isLoading ? <Spinner/> : <MdCreate/>}/>
            </form>
        </div>
