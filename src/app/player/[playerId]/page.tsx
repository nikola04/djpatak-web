...
export default function Home({
  ...
  } = useCurrentTrack(playerId);
  const {
    data: queue,
    setData: setQueue,
    loading: queueLoading,
  } = usePlayerQueue(playerId);
  ...
  const deleteTrack = async (track: QueueTrack) => {
    try {
      ...
      setQueue((queue) =>
        queue.filter((queueTrack) => queueTrack.queueId !== track.queueId),
      );
        ...
        {!queueLoading && queue.length == 0 ? (
          <div>
            <p className="text-white-default">
              No tracks. You should start searching.
            </p>
          </div>
        ) : (
          <PlayerQueue
...
function PlayerQueue({
  ...
  if (loading)
    return [...new Array(10)].map((val, ind) => (
      <PlayerQueueTrackSceleton key={ind} />
    ));
...
function PlayerQueueTrackSceleton() {
  return (
    <div className="flex w-full p-2 my-0.5">
      <div
        className="relative rounded overflow-hidden bg-blue-grayishanimate-pulse"
        style={{ width: "48px", height: "48px" }}
      ></div>
      <div
        className="flex flex-col pl-2.5 justify-around"
        style={{ height: "48px" }}
      >
  ...
  return (
    ...
    >
      <div className="flex w-full">
        ...
        >
          ...
          >
            <div className="flex bg-blue-light rounded-full w-10/12 aspect-square items-center justify-center">
              {(!current || isPaused) && (
                <FaPlay className="text-white-default text-lg ml-1" />
              )}
              <div
                className={`p-0.5 pl-1 hidden ${current && !isPaused && "!block"}`}
                ref={soundwaveBox}
              ></div>
            </div>
          </div>
        ...
        <div
          className="flex flex-col pl-2.5 justify-around w-[280] flex-grow flex-shrink"
        ...
        <div className="flex items-center justify-center">
          <SmallIconButton
            title="Remove Song"
            onClick={onDelete}
            icon={<TfiTrash className="text-lg" />}
          />
        </div>
      </div>
    </div>
