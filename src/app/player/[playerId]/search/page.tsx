...
export default function SearchPage({
	...
	const { soundcloudTracks, loading } = useSoundcloudTracksSearch(query, tracksLimit, true);
	const { sortedPlaylists, setPlaylists } = useUserPlaylists();
	const memoizedButtons = useCallback(
		({ track }: { track: DbTrack }) => (
			<SearchTrackButtons guildId={playerId} playlists={sortedPlaylists} setPlaylists={setPlaylists} track={track} />
		),
	...
	return (
		<div className="p-4">
			<div className="">
				...
				<TracksList guildId={playerId} tracks={soundcloudTracks} loading={loading} Buttons={memoizedButtons} />
			</div>
		</div>
	);
}
