...
function Guild({ data }: {
    ...
    return <div className="flex flex-col p-4 bg-blue-grayish rounded-lg w-72 shadow">
        <div className="flex items-center">
            <GuildIcon guild={data} size={64} className="rounded-md flex-grow-0 flex-shrink-0 shadow" classNameNoImageText="!text-base" classNameNoImage="border-blue-light border-1" style={{ flexBasis: "64px" }}/>
...
function UserGuildsSceleton(){
    return <div className="grid py-4 gap-8 px-4 w-full justify-center" style={{ gridTemplateColumns: "repeat(auto-fit, 18rem)"}}>
        {[...Array(7)].map((_, ind) => <div key={ind} className="bg-blue-grayish rounded-md w-72 h-[150px] shadow animate-pulse"></div>) }
    </div>
}
